...
public class UsuarioDAO implements Persistable<Usuario> {
	...
	private static final String SQL_UPDATE = "UPDATE `usuario` SET `id`= ?,`nombre`= ?,`apellido1`= ?,`apellido2`= ?,`edad`= ?,`email`= ?,`dni`= ?,`puesto`= ?,`password`= ?,`imagen`= ? WHERE `id` = ?";
	private static final String SQL_CREATE = "INSERT INTO `usuario`(`id`, `nombre`, `apellido1`, `apellido2`, `edad`, `email`, `dni`, `puesto`, `password`, `imagen`) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
	...
	@Override
	public boolean update(Usuario onjeto) {
		// TODO Auto-generated method stub
		return false;
	}
	...
	/**
	 * Cuenta los usuarios totoales de la tabla
	 * 
